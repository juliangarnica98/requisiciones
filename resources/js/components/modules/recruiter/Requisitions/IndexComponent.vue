                                    ...
                                    <div class="col-md-2 text-center">
                                        ...
                                        <div class="col-md-12 h4">
                                            <div class="row d-flex justify-content-center">
                                                ...
                                                <div class="col-md-2">
                                                    <router-link class="link text-white" :to="{name:'/recruiter/requisicion',params:{area: area, id: rq.id}}" aria-expanded="false" >
                                                        <i class="fas fa-eye"></i>
                                                    </router-link>
                                                </div>
                                                <div class="col-md-2" >
                                                    <div v-if="rq.status== 'CERRADA' && rq.nombre_ingreso == null">
                                                        <a class="link text-white"  data-bs-toggle="modal" :data-bs-target="'#'+area+'-'+rq.id+'-'+rq.status"><i class="fas fa-user-check"></i></a>    
                                                        <Edit2 @traerdata="getRequisitions4"  :estado="rq.status" :area="area" :id="rq.id"/>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
    ...
    export default {
        ...
        methods:{
            ...
            getRequisitions3(page = 1){
                axios.get('/recruiter/getrequisition2?page='+page)
                .then((res) => { 
                    this.listaRequisition = res.data.store;  
                    this.listaRequisitionAdmin = res.data.admin; 
                    this.listaRequisitionCedi = res.data.cedi;
            ...
            traerJefesRegional(page = 1){
                axios.get(`/recruiter/getjefes/`+this.filtro_regional+`/`+this.area+'?page='+page)
                .then((res) => { 
