                            ...
                            <div class="card border-none" :class="{ estado_abierto: rq.status == 'ABIERTA', estado_cerrado: rq.status == 'CERRADA' ,estado_engestion: rq.status == 'EN GESTION',estado_cancelado: rq.status == 'CANCELAR',estado_cancelado: rq.status == 'CANCELADA',estado_suspendido: rq.status == 'SUSPENDIDA'  }">
                                <div class="card-body">
                                    <div class="row ">
                                        ...
                                        <div class="col-md-2 text-center" v-if="rq.aprobacion ===0 && rq.rechazo ===0 && rq.status =='ABIERTA'"><b style="text-transform: uppercase;" >SIN ASIGNACIÓN</b> </div>
                                        ...
                                        <div class="col-md-1 text-center d-flex justify-content-center aling-items-center" v-else><b> 
                                            <div class="" v-if="rq.status == 'EN GESTION'">
                                                {{ rq.status }} - {{ rq.substate }}
                                            </div>
                                            <div class="" v-else>
                                                ...
                                                <div class="col-md-12 h4">
                                                    <div class="row d-flex justify-content-center">
                                                        ...
                                                        <div class="col-md-2 text-white">
                                                            <router-link class="link text-white" :to="{name:'/specialist/requisicion',params:{area: area, id: rq.id}}" aria-expanded="false" >
                                                                <span class="" >
    ...
    export default {
        ...
        methods:{
            ...
            getRequisitions3(page = 1){
                axios.get('/specialist/getrequisition2?page='+page)
                .then((res) => { 
                    this.listaRequisition = res.data.store;  
                    this.listaRequisitionAdmin = res.data.admin; 
                    this.listaRequisitionCedi = res.data.cedi;
            ...
            filtrarJefe(page = 1){
                ...
                if (this.filtro_jefe){
                    axios.get(`/specialist/getjefes2/`+this.filtro_regional+`/`+this.area+`/`+this.filtro_jefe+'?page='+page)
                    .then((res) => { 
                        this.listaRequisitionStore= res.data.store;
                        this.listaRequisition = this.listaRequisitionStore;
                    });
            ...
            filtrarEstado(page = 1){
                ...
                if (this.filtro_jefe == '') {
                    axios.get(`/specialist/getfiltro/`+this.area+`/sin_jefe/`+this.filtro_estado+`/`+this.filtro_regional+'?page='+page)
                    .then((res) => { 
                        if (res.data.store) {
                            this.listaRequisitionStore= res.data.store;
                            this.listaRequisition = this.listaRequisitionStore;
                    ...
                    axios.get(`/specialist/getfiltro/`+this.area+`/`+this.filtro_jefe+`/`+this.filtro_estado+`/`+this.filtro_regional+'?page='+page)
                    .then((res) => { 
